...
public class FieldSojaFinder implements SojaFinderInterface<Field> {
  @Override
  public Soja[] findSoja(Field field) {
    return findSoja(field, new HashSet<>());
  }

  private Soja[] findSoja(Field field, Set<Class<? extends Annotation>> visited) {
    if (field == null) {
      return new Soja[0];
    }
    ...
    if (field.isAnnotationPresent(Soja.class)) {
      sojas.add(field.getAnnotation(Soja.class));
    }
