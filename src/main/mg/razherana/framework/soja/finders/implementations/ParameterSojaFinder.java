...
public class ParameterSojaFinder implements SojaFinderInterface<Parameter> {
  @Override
  public Soja[] findSoja(Parameter parameter) {
    return findSoja(parameter, new HashSet<>());
  }

  private Soja[] findSoja(Parameter parameter, Set<Class<? extends Annotation>> visited) {
    if (parameter == null) {
      return new Soja[0];
    }
    ...
    if (parameter.isAnnotationPresent(Soja.class)) {
      sojas.add(parameter.getAnnotation(Soja.class));
    }
