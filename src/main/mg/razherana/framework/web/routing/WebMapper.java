...
public class WebMapper {
  ...
  private WebRouteContainer findRouteMethod(HttpMethod httpMethod, String path) {
    for (var controller : webFinder.getControllerContainers()) {
      var controllerPath = normalizePath(controller.getControllerAnnotation().value());
      ...
      for (var routing : controller.getRoutingContainers()) {
        if (!routing.getHttpMethod().equals(httpMethod))
          continue;

        var fullRoutingPath = normalizePath(controllerPath + "/" + normalizePath(routing.getPath()));

        var dataMatch = checkPathMatchAndExtractParameters(fullRoutingPath, path, routing.getMethodReflection(),
  ...
  private HashMap<String, String> extractPathParameters(String fullRoutingPath, String path) {
    ...
    for (int i = 0; i < routingSegments.length; i++) {
      ...
      if (routingSegment.startsWith("[") && routingSegment.endsWith("]")) {
        var paramName = routingSegment.substring(1, routingSegment.length() - 1).split(":", 2)[0];
  ...
  private String matchSegmentAndReturn(String routingSegment, String pathSegment) {
    if (routingSegment.startsWith("[") && routingSegment.endsWith("]")) {
      ...
      var splitted = routingSegment.substring(1, routingSegment.length() - 1).split(":", 2);
      if (splitted.length == 2) {
        var regex = splitted[1];
  ...
  private WebRouteContainer checkPathMatchAndExtractParameters(String fullRoutingPath, String path, Method method,
    ...
    if (pathParameters != null) {
      return new WebRouteContainer(method,
          controllerInstance,
          pathParameters,
          webFinder.getSojaClasses().get(controllerInstance.getClass()));
    }
